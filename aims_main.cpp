...
int main() {
	while (noAdmins()) {
		...
		err(isLegal(u_init), "Illegal username entered.");
		string p_init = get_str("Enter admin password: ", 0);
		err(isLegal(p_init), "Illegal password entered.");
	...
	while (!isUser(u, l) || !pass(p, u, l)) {
		u = get_str("User doesn't exist or password is incorrect!\nUsername: ", 0);
	...
	if (l == 0) {
		...
		while (op != 5) {
			...
			if (op == 0) {
				...
				err(isLegal(p), "Entered password is invalid.");
				S.passwd(p);
